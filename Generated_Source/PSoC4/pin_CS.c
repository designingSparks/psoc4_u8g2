...
#include "pin_CS.h"
    ...
    #define pin_CS_SetP4PinDriveMode(shift, mode)  \
    do { \
        pin_CS_PC =   (pin_CS_PC & \
                                (uint32)(~(uint32)(pin_CS_DRIVE_MODE_IND_MASK << \
                                (pin_CS_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (pin_CS_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define pin_CS_SetP4PinDriveMode(shift, mode)  \
        do { \
            pin_CS_USBIO_CTRL_REG = (pin_CS_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(pin_CS_DRIVE_MODE_IND_MASK << \
                                    (pin_CS_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (pin_CS_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		pin_CS_SetP4PinDriveMode(pin_CS__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(pin_CS_DR & (uint8)(~pin_CS_MASK));
    drVal = (drVal | ((uint8)(value << pin_CS_SHIFT) & pin_CS_MASK));
    pin_CS_DR = (uint32)drVal;
}
...
{
    return (uint8)((pin_CS_PS & pin_CS_MASK) >> pin_CS_SHIFT);
}
...
{
    return (uint8)((pin_CS_DR & pin_CS_MASK) >> pin_CS_SHIFT);
}
...
void pin_CS_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  pin_CS_INTCFG & (uint32)(~(uint32)position);
    pin_CS_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(pin_CS_INTSTAT & pin_CS_MASK);
	pin_CS_INTSTAT = maskedStatus;
    return maskedStatus >> pin_CS_SHIFT;
}
