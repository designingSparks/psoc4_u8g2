...
#include "SS.h"
...
#if defined(SS__PC)
    #define SS_SetP4PinDriveMode(shift, mode)  \
    do { \
        SS_PC =   (SS_PC & \
                                (uint32)(~(uint32)(SS_DRIVE_MODE_IND_MASK << \
                                (SS_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (SS_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define SS_SetP4PinDriveMode(shift, mode)  \
        do { \
            SS_USBIO_CTRL_REG = (SS_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(SS_DRIVE_MODE_IND_MASK << \
                                    (SS_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (SS_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		SS_SetP4PinDriveMode(SS__0__SHIFT, mode);
    }
#endif
...
void SS_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  SS_INTCFG & (uint32)(~(uint32)position);
    SS_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
