...
*  associated v_EEPROM_Init() function.
*/
cy_stc_eeprom_config_t const v_EEPROM_config =
{
    256U,
    1U,
    0U,
...
*/
cy_stc_eeprom_context_t v_EEPROM_context;

/* For PSoC 6 place EEPROM into Emulated EEPROM flash area */
#if ((CY_PSOC6) && (v_EEPROM_STORAGE_EM_EEPROM))

    CY_SECTION(".cy_em_eeprom") CY_ALIGN(CY_FLASH_SIZEOF_ROW)
    const uint8 v_EEPROM_em_EepromStorage[v_EEPROM_ACTUAL_SIZE] = {0u};

#endif /* ((CY_PSOC6) && (v_EEPROM_STORAGE_EM_EEPROM)) */


/*******************************************************************************
* Function Name: v_EEPROM_Init
********************************************************************************
*
...
cy_en_em_eeprom_status_t v_EEPROM_Init(uint32 startAddress)
{
    ...
    config.blockingWrite =      v_EEPROM_config.blockingWrite;
    config.eepromSize =         v_EEPROM_config.eepromSize;
    config.redundantCopy =      v_EEPROM_config.redundantCopy;
    config.wearLevelingFactor = v_EEPROM_config.wearLevelingFactor;
    
#if ((CY_PSOC6) && (v_EEPROM_STORAGE_EM_EEPROM))
    /* For PSoC 6, if Emulated EEPROM is selected - override the eeprom start 
    * address.
    */
    config.userFlashStartAddr = (uint32)v_EEPROM_em_EepromStorage;
#else
    config.userFlashStartAddr = startAddress;
#endif /* ((CY_PSOC6) && (v_EEPROM_STORAGE_EM_EEPROM)) */
        
    ret = Cy_Em_EEPROM_Init(&config, &v_EEPROM_context);
